...
RSpec.describe Transcoder do
  ...
  describe '#transcode' do
    ...
    context 'transcoding to he-aac' do
      it 'transcodes the file with correct params' do
        ...
        transcoder = Transcoder.new(file: file, title: title, contributor: contributor)
        ...
        expect(file_movie).to receive(:transcode).with(
          output,
          {
            ...
            audio_bitrate: "48",
            custom: ["-metadata", "title=#{title}", "-metadata", "artist=#{contributor}"]
          }
        )
      ...
      it 'returns the file duration' do
        ...
        expect(transcoder.transcode(output: output, format: Transcoder::HE_AAC)).to eq file_movie.duration
      end
    end
    ...
    context 'transcoding to mp3' do
      ...
      it 'returns the file duration' do
        ...
        expect(transcoder.transcode(output: output, format: Transcoder::MP3_128)).to eq file_movie.duration
      end
    end
  end
  ...
  describe '#get_duration' do
    it 'returns the file duration' do
      ...
      expect(transcoder.get_duration).to eq file_movie.duration
    end
  end
  ...
  describe 'initialize' do
    context 'when passed file does not exist' do
      it 'raises a StandardError' do
        expect {
          Transcoder.new(file: 'wrong-path', title: title, contributor: contributor)
          }.to raise_error(StandardError, "File does not exist: wrong-path")
      end
    end
  end
end
