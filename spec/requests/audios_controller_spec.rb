...
describe Api::V1::AudiosController do
  ...
  before(:each) do |example|
    ...
    unless example.metadata[:skip_auth]
      allow_any_instance_of(Api::V1::AudiosController)
    ...
    context 'a non-last chunk is received' do
      it 'does not combine all the chunks' do
        ...
        file = Rack::Test::UploadedFile.new(
          Rails.root.join('spec', 'fixtures', test_file), 'audio/wav'
        )
        allow(Dir).to receive(:[]).and_return(
          [
            'chunk_file_directory/lalala1.wav.part1',
            'chunk_file_directory/lalala2.wav.part2'
          ]
        )
  ...
  describe 'INDEX' do
    ...
    context 'user_id is passed in' do
      let(:uploader2) do
        FactoryBot.create(:user, uid: 'uid2', nickname: 'some other person')
      end
  ...
  describe 'UPDATE' do
    ...
    it 'updates appropriate field' do
      ...
      put "#{AUDIO_API_ENDPOINT}#{audio.id}",
          params: { title: new_title }.to_json
    ...
    it 'creates new contributors' do
      expect do
        put "#{AUDIO_API_ENDPOINT}#{audio.id}",
            params: { contributors: 'Mary Lantol, Badielin Mand' }.to_json
      end.to change { Contributor.count }.by(2)
    end

    it 'returns a 404 if audio not found' do
      put "#{AUDIO_API_ENDPOINT}#{audio.id + 99}",
          params: { title: 'new title here' }
  ...
  describe 'DESTROY' do
    ...
    it 'does not delete an unfound file' do
      ...
      end.to change { Audio.count }.by(0)
